class MahjongGame {
    constructor() {
        ...
        this.wall = []; // Track the wall of tiles
        this.deadWall = []; // Track the dead wall (4 tiles)
        this.doraIndicators = []; // Track dora indicators
        this.currentDoraIndex = 0; // Track current dora indicator
        
        // Initialize players with proper Mahjong game state
        for (let i = 0; i < this.numPlayers; i++) {
            this.players.push({
                ...
                concealed: true,
                score: 0,
                riichi: false, // Track if player has declared riichi
                canRiichi: false, // Track if player can declare riichi
                lastAction: null, // Track last action (draw, discard, etc.)
                lastDraw: null // Track last drawn tile
            });
        }
    ...
    setupWall() {
        // Create the wall (136 tiles)
        this.wall = [...this.tiles];
        
        // Set up dead wall (4 tiles)
        this.deadWall = this.wall.splice(0, 4);
        
        // Set first dora indicator
    ...
    updateView() {
        ...
        nextTurnButton.disabled = this.currentPlayer === this.activeView && 
            (!this.players[this.currentPlayer].hasDrawn || !this.players[this.currentPlayer].hasDiscarded);
    }
    ...
    drawTile() {
        ...
        currentPlayer.lastDraw = drawnTile;
        ...
        currentPlayer.lastAction = 'draw';
        
        // Check if player can declare riichi
        this.checkRiichiEligibility(this.currentPlayer);
    ...
    updateUI() {
        ...
        // Update dora indicators display
        const doraContainer = document.querySelector('.dora-indicators') || document.createElement('div');
        doraContainer.className = 'dora-indicators';
        doraContainer.innerHTML = `
            <h3>Dora Indicators</h3>
            <div class="dora-list">
                ${this.doraIndicators.map(indicator => {
                    const doraTile = this.getDoraTile(indicator);
                    return `
                        <div class="dora-indicator">
                            <div class="indicator">${this.createTileElement(indicator).outerHTML}</div>
                            <div class="dora">${this.createTileElement(doraTile).outerHTML}</div>
                        </div>
                    `;
                }).join('')}
            </div>
        ...
        if (!document.querySelector('.dora-indicators')) {
            document.querySelector('.game-container').appendChild(doraContainer);
        }

        // Add riichi button if eligible
        if (currentPlayer.canRiichi && !currentPlayer.riichi) {
            const riichiButton = document.createElement('button');
            riichiButton.id = 'riichi-button';
            riichiButton.className = 'control-button';
            riichiButton.textContent = 'Declare Riichi';
            riichiButton.addEventListener('click', () => this.declareRiichi(this.currentPlayer));
            document.querySelector('.game-controls').appendChild(riichiButton);
        } else {
            const existingRiichiButton = document.getElementById('riichi-button');
            if (existingRiichiButton) {
                existingRiichiButton.remove();
            }
        }
    }
    ...
    calculateScore(hand) {
        ...
        // Add points for dora
        const doraCount = this.calculateDoraCount(hand);
        score += doraCount * 10;
        
        // Add points for riichi
        if (player.riichi) {
            score += 10;
        }
        ...
        // Apply multipliers
        if (player.riichi) score *= 2;
        if (this.checkConcealedHand(hand)) score *= 2;
    ...
    checkRiichiEligibility(playerIndex) {
        const player = this.players[playerIndex];
        
        // Can't declare riichi if:
        // 1. Already declared riichi
        // 2. Has less than 1000 points
        // 3. Has already discarded a tile this turn
        // 4. Has any open melds
        if (player.riichi || 
            player.score < 1000 || 
            player.hasDiscarded || 
            player.melds.some(meld => !meld.concealed)) {
            player.canRiichi = false;
            return;
        }

        // Check if hand is one tile away from winning
        const originalHand = [...player.hand];
        const possibleWinningTiles = this.findPossibleWinningTiles(originalHand);
        
        player.canRiichi = possibleWinningTiles.length > 0;
    }

    findPossibleWinningTiles(hand) {
        const possibleTiles = [];
    ...
    // Add riichi declaration
    declareRiichi(playerIndex) {
        const player = this.players[playerIndex];
        
        if (!player.canRiichi) {
            this.showNotification('Cannot declare riichi!');
            return;
        }
        
        if (player.score < 1000) {
            this.showNotification('Need at least 1000 points to declare riichi!');
            return;
        }
        ...
        // Place riichi bet
        this.showNotification('Riichi declared!');
        this.updateUI();
    }
    ...
    updateDoraIndicator() {
        ...
        this.currentDoraIndex++;
        if (this.currentDoraIndex < this.deadWall.length) {
            this.doraIndicators.push(this.deadWall[this.currentDoraIndex]);
        }
    }

    // Get dora tile for a given indicator
    getDoraTile(indicator) {
        if (indicator.type === 'number') {
            // For number tiles, next number (9 -> 1)
            const nextNumber = indicator.number === 9 ? 1 : indicator.number + 1;
            return {
                type: 'number',
                suit: indicator.suit,
                number: nextNumber,
                display: `${nextNumber}${indicator.suit}`
            };
        } else if (indicator.type === 'wind') {
            // For winds: 東 -> 南 -> 西 -> 北 -> 東
            const winds = ['東', '南', '西', '北'];
            const currentIndex = winds.indexOf(indicator.value);
            const nextWind = winds[(currentIndex + 1) % 4];
            return {
                type: 'wind',
                suit: '風',
                value: nextWind,
        ...
        } else if (indicator.type === 'dragon') {
            // For dragons: 中 -> 發 -> 白 -> 中
            const dragons = ['中', '發', '白'];
            const currentIndex = dragons.indexOf(indicator.value);
            const nextDragon = dragons[(currentIndex + 1) % 3];
            return {
                type: 'dragon',
                suit: '龍',
                value: nextDragon,
    ...
    // Calculate dora count for a hand
    calculateDoraCount(hand) {
        let doraCount = 0;
        
        for (let indicator of this.doraIndicators) {
            const doraTile = this.getDoraTile(indicator);
            doraCount += hand.filter(tile => this.isSameTile(tile, doraTile)).length;
        }
